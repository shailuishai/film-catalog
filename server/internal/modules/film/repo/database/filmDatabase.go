...
func (db *FilmDatabase) GetAllFilmsWithDetails(page, pageSize int) ([]*f.FilmDTO, error) {
	var films []*f.Film

	// Основной запрос для получения фильмов с пагинацией
	if err := db.db.Preload("Genres").Preload("Actors").
		Offset((page - 1) * pageSize).Limit(pageSize).
		Find(&films).Error; err != nil {
		db.log.Error("failed to get films with details", "error", err)
		return nil, f.ErrInternal
	}
